...
**`local.conf`** (key settings — full file in §1.6 deliverables):
...
| `BB_NUMBER_THREADS` | `4` | Parallel BitBake tasks |
| `PARALLEL_MAKE` | `-j 4` | Parallel compilation |
| `DISTRO_FEATURES:append` | `opengl systemd` | Adds OpenGL (for Qt) and the systemd init system |
| `VIRTUAL-RUNTIME_init_manager` | `systemd` | Selects systemd as init manager — required for the auto-start bonus |
| `DISTRO_FEATURES_BACKFILL_CONSIDERED` | `sysvinit` | Drops the default SysV init so only systemd is used |
| `PREFERRED_VERSION_qtbase` | `5.15%` | Pins Qt5 to the 5.15 series |
| `INHERIT` | `rm_work` | Frees disk space by deleting work files after each build |
| `EXTRA_IMAGE_FEATURES` | `debug-tweaks` | Empty root password for easy QEMU login |

> **Why systemd?** Poky's default init is SysV. The bonus requirement (server auto-starts on boot) relies on a systemd service unit, so systemd is enabled as the init manager here and `sysvinit` is backfilled out.
...
graph BT
    subgraph "Layer Priority Stack"
        ...
        L5["meta-multimedia — multimedia support"]
        L6["meta-python — Python support"]
        L7["meta-raspberrypi — Raspberry Pi BSP"]
        L8["meta-qt5 — Qt5 framework"]
        L9["meta-chat — our application layer"]
...
# Free up space after each package builds
INHERIT += "rm_work"
EXTRA_IMAGE_FEATURES += "debug-tweaks"
```
...
BBLAYERS ?= " \
  ...
  /home/hp/yocto/meta-openembedded/meta-multimedia \
...
LAYERDEPENDS_chat = "core qt5-layer openembedded-layer"
LAYERSERIES_COMPAT_chat = "kirkstone"
```

> The collection is named `chat` (set by `BBFILE_COLLECTIONS`). `LAYERDEPENDS` declares that this layer requires the core layer plus the `qt5-layer` (meta-qt5) and `openembedded-layer` (meta-oe) collections, so BitBake refuses to parse it unless those layers are present.
...
layer                 path                                    priority
==========================================================================
chat                  /home/hp/yocto/meta-chat                6
```
...
```bitbake
SUMMARY = "Qt5 GUI TCP Chat Client"
DESCRIPTION = "A simple TCP chat client with Qt5 GUI, built for CSE412 Project 2."
LICENSE = "MIT"
...
```bitbake
SUMMARY = "Qt5 GUI TCP Chat Server"
DESCRIPTION = "A simple multi-client TCP chat server with Qt5 GUI, built for CSE412 Project 2."
LICENSE = "MIT"
LIC_FILES_CHKSUM = "file://${COMMON_LICENSE_DIR}/MIT;md5=0835ade698e0bcf8506ecda2f7b4f302"
...
do_install() {
    ...
    install -m 0644 ${WORKDIR}/chat-server.service ${D}/lib/systemd/system/chat-server.service

    install -d ${D}/lib/systemd/system/multi-user.target.wants
    ln -sf ../chat-server.service ${D}/lib/systemd/system/multi-user.target.wants/chat-server.service
}
...
```bitbake
SUMMARY = "Minimal image containing the Qt5 chat server"

require recipes-core/images/core-image-minimal.bb

IMAGE_INSTALL:append = " chat-server qtbase qtbase-plugins qtbase-tools liberation-fonts "
...
```bitbake
SUMMARY = "Minimal image containing the Qt5 chat client"

require recipes-core/images/core-image-minimal.bb

IMAGE_INSTALL:append = " chat-client qtbase qtbase-plugins qtbase-tools liberation-fonts "

IMAGE_FEATURES += "ssh-server-openssh"
IMAGE_FEATURES += "allow-empty-password"
```

Both images start from `core-image-minimal` and add the application package, the Qt5 runtime (`qtbase`, `qtbase-plugins` — including the `offscreen` platform plugin — and `qtbase-tools`), and `liberation-fonts` so Qt has a usable font. `ssh-server-openssh` and `allow-empty-password` make each VM reachable over SSH for testing.
...
flowchart TD
    ...
    H --> I([Output: chat-server / chat-client image .ext4])
